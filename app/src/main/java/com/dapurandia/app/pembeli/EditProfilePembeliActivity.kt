...
class EditProfilePembeliActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        editTextPasswordSaatIni = findViewById(R.id.editPasswordSaatIni)
        ...
        buttonSimpan.setOnClickListener {
            ...
            if (!android.util.Patterns.EMAIL_ADDRESS.matcher(emailBaru).matches()) {
                Toast.makeText(this, "Format email tidak valid", Toast.LENGTH_SHORT).show()
                return@setOnClickListener
            }

            if (emailBaru != currentEmail && passwordSaatIni.isBlank()) {
                Toast.makeText(this, "Masukkan password saat ini untuk mengganti email", Toast.LENGTH_SHORT).show()
                return@setOnClickListener
            }

            if (selectedImageUri != null) {
                uploadPhotoToCloudinary(selectedImageUri!!) { urlFotoBaru ->
                    saveProfileData(namaBaru, noHPBaru, emailBaru, passwordSaatIni, urlFotoBaru)
                }
            } else {
                saveProfileData(namaBaru, noHPBaru, emailBaru, passwordSaatIni, uploadedPhotoUrl)
            }
        }
    }
        ...
        firestore.collection("pembeli").document(userId)
            ...
            .addOnSuccessListener { doc ->
                if (doc.exists()) {
                    ...
                    currentEmail = doc.getString("email") ?: auth.currentUser?.email.orEmpty()
    ...
    private fun saveProfileData(
        ...
        val saveToFirestore = {
            updateProfileDocument(userId, data, progressDialog)
        }

        if (email != currentEmail) {
            val credential = EmailAuthProvider.getCredential(currentEmail, passwordSaatIni)
            user.reauthenticate(credential)
                .addOnSuccessListener {
                    user.updateEmail(email)
                        .addOnSuccessListener { saveToFirestore() }
                        .addOnFailureListener { error ->
                            showUpdateError(progressDialog, "Gagal mengganti email: ${error.message}")
                        }
                }
                .addOnFailureListener { error ->
                    showUpdateError(progressDialog, "Password saat ini salah atau sesi login perlu diperbarui: ${error.message}")
                }
        } else {
            saveToFirestore()
        }
    ...
    private fun updateProfileDocument(
        userId: String,
        data: Map<String, Any>,
        progressDialog: cn.pedant.SweetAlert.SweetAlertDialog
    ) {
        firestore.collection("pembeli").document(userId)
            .update(data)
            .addOnSuccessListener {
                currentEmail = data["email"] as? String ?: currentEmail
            ...
            .addOnFailureListener { error ->
                showUpdateError(progressDialog, "Gagal memperbarui profil: ${error.message}")
            }
    }
